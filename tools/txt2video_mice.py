...
def txt2img(visual_path="vis_gt", 
            show_video_name='name',
            img_path='mice/train',
            txt_path='mice/train/gt/gt.txt',
            args=None):
    ...
    ori_img_paths = gb.glob(img_path + "/" + show_video_name + "/img1/*.png")
    for ori_img_path in sorted(ori_img_paths):
        ...
        if img_id in txt_dict:
            for bbox in txt_dict[img_id]:
                ...
                cv2.putText(img, "{}".format(int(bbox[4])), (int(bbox[0]), int(bbox[1])-5), cv2.FONT_HERSHEY_SIMPLEX, 0.75, color_list[bbox[4]%79].tolist(), 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_path', default='../datasets/mice', type=str, help='path to dancetrack dataset')
    ...
    parser.add_argument('--tracker', default='mice_track', type=str, help='name of tracker')
    ...
    for show_video_name in show_video_names:
        if '.ipy' in show_video_name or 'seq' in show_video_name or 'DS_' in show_video_name:
            continue
        if args.gt:
            txt_path = 'mice/' + args.split + '/' + show_video_name + '/gt/gt.txt'
        else:
            # txt_path = 'mice_'+args.split + '/' + args.tracker + '/' + show_video_name + '.txt'
            txt_path = './YOLOX_outputs/test/mice_val/' + show_video_name + '.txt'
