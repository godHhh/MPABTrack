...
def write_results_no_score_have_class(filename, results):
        ...
        for frame_id, tlwhs, track_ids,classes in results:
            for tlwh, track_id,cls in zip(tlwhs, track_ids,classes):
                if track_id < 0:
                    continue
                x1, y1, w, h = tlwh
                line = save_format.format(frame=frame_id, id=track_id, x1=round(x1, 1), y1=round(y1, 1), w=round(w, 1), h=round(h, 1),c=round(cls,1))
                f.write(line)
    logger.info('save results to {}'.format(filename))
